      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="factura in facturas" :key="factura.internalId">
            <td>{{ factura.fecha_remision }}</td>
            <td>{{ factura.cliente }}</td>
            <td>{{ factura.nro_remision }}</td>
  ...
  export default {
    ...
    methods: {
      ...
      async obtenerFacturas() {
        let facturas;
        await axios.get(`${process.env.VUE_APP_API_URL}/remisiones/tosco/`).then((response) => {
          facturas = response.data; // Asignamos las facturas desde la respuesta
        });
  
        if (this.fechaInicio || this.fechaFin) {
          facturas = facturas.filter(factura => {
            const fechaFactura = new Date(factura.fecha_remision);
        ...
        this.facturas = facturas.sort((a, b) => this.ordenAscendente ? new Date(a.fecha_remision) - new Date(b.fecha_remision) : new Date(b.fecha_remision) - new Date(a.fecha_remision));
      },
      ordenarPorFecha() {
